    ...
    inline ApplicationArguments parse_arguments(int argc, char *argv[])
    {
        ...
        while (arg_processing < argc) {
            ...
            || strcmp(argv[arg_processing], "--verbosity") == 0)) {
                ...
                arg_processing += 2;
            }  else if ((argc > arg_processing)
            && (strcmp(argv[arg_processing], "-p") == 0
            || strcmp(argv[arg_processing], "--peers") == 0)) {
                config_peers = true;
                arg_processing += 1;
        ...
        if (show_usage) {
            ...
            "                               Range: 0-3 \n"
            "                               Default: 1\n"
            "    -p, --peers                Use the hard-coded initial peers list"
            << std::endl;
        }

        return ApplicationArguments(parse_result, domain_id, sample_count, verbosity, config_peers);
    }
